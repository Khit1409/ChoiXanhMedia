...
import { getProductByUrl } from "@/redux/slices/productSlice";
...
export default function Product() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const results = await dispatch(getProductByUrl({ producturl: url }));
        if (getProductByUrl.fulfilled.match(results)) {
          setMess("Thành công");
        }
      } catch (error) {
        console.error(error);
